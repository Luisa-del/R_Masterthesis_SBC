#/////////////////////////////////////////////
# Land cover classification Southeast Vietnam
# Luisa Pflumm
# Masterthesis
# 22.03.2023
#/////////////////////////////////////////////
...
# convert raster values for each class to spdf 
...
r.spgrd2<-as(r2,"SpatialPointsDataFrame")
...
# sample equal random stratified points
set.seed(22)
...
# combine sample points into one dataset
smpl_equal <- rbind(r_samp1,
                    r_samp2,
                    r_samp3,
                    r_samp4,
                    r_samp5,
...
#mapview(aoi, alpha.regions = 0) + mapview(td, zcol = "class_id") + mapview(smpl_equal)
plot(aoi$geometry)
plot(smpl_equal$geometry, add=T)


smpl_equal <- st_read("D:/Luisa/Rsuperclass/SC_prob_10m/smpl_SVN10_td2.8_10000.shp")




## 2. weighted stratified random sampling ---------------------------------------------------------------------------


# sample weighted stratified random points
# 10000 samples for evergreen forest (biggest class), based on that calculate sample size per class
set.seed(22)
selectedPoints1 = sample(1:length(r.spgrd1[[1]]), 10000)
set.seed(22)
selectedPoints2 = sample(1:length(r.spgrd2[[1]]), round((number_of_2s * 100 / number_of_1s) * 10000 / 100))
set.seed(22)
selectedPoints3 = sample(1:length(r.spgrd3[[1]]), round((number_of_3s * 100 / number_of_1s) * 10000 / 100))
set.seed(22)
selectedPoints4 = sample(1:length(r.spgrd4[[1]]), round((number_of_4s * 100 / number_of_1s) * 10000 / 100))
set.seed(22)
selectedPoints5 = sample(1:length(r.spgrd5[[1]]), round((number_of_5s * 100 / number_of_1s) * 10000 / 100))
set.seed(22)
selectedPoints6 = sample(1:length(r.spgrd6[[1]]), round((number_of_6s * 100 / number_of_1s) * 10000 / 100))
set.seed(22)
selectedPoints7 = sample(1:length(r.spgrd7[[1]]), round((number_of_7s * 100 / number_of_1s) * 10000 / 100))
set.seed(22)
selectedPoints8 = sample(1:length(r.spgrd8[[1]]), round((number_of_8s * 100 / number_of_1s) * 10000 / 100))
set.seed(22)
selectedPoints9 = sample(1:length(r.spgrd9[[1]]), round((number_of_9s * 100 / number_of_1s) * 10000 / 100))
...
#smpl_weighted <- st_read("D:/Luisa/Rsuperclass/SC_prob_10m/smpl_SVN10_td2.8_10000_weighted.shp")

#mapview(aoi, alpha.regions = 0) + mapview(td, zcol = "class_id") + mapview(smpl_weighted)
plot(aoi$geometry)
plot(smpl_weighted$geometry, add=T)
...
sf_all <- rbind(st_as_sf(r.spgrd1),
                st_as_sf(r.spgrd2),
                st_as_sf(r.spgrd3),
                st_as_sf(r.spgrd4),
                st_as_sf(r.spgrd5),
...
# take random samples
set.seed(22)
selectedPoints_all = sample(1:length(sf_all[[1]]), 90000)
smpl_all = sf_all[selectedPoints_all,]

# rename columns
names(smpl_all) <- c("class_id", "geometry")



#smpl_all <- st_read("D:/Luisa/Rsuperclass/SC_prob_10m/smpl_SVN10_td2.8_10000_sample_all.shp")

#mapview(aoi, alpha.regions = 0) + mapview(td, zcol = "class_id") + mapview(smpl_all)
plot(aoi$geometry)
plot(smpl_all$geometry, add=T)
...
# select samples for land cover classification
smpl <- smpl_weighted
# smpl <- smpl_all
smpl <- smpl_equal
